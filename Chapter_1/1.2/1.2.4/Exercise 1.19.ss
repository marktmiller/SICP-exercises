; There is a clever algorithm for computing Fibonacci numbers in a
...
; of p = 0 and q = 1 in a family of transformations T(pq), where T(pq)
; transforms the pair (a,b) according to:
;
; a<--bq + aq + ap and b<--bp + aq
;
; Show that if we apply such a transformation T(pq) twice,
; the effect is the same as using a single transformation T(p'q') of the
...
;                          (- count 1)))))
;
; By observing the behavior of transformations T and T(pq), I came up with:
;
; if p + q = 1
;    p' = 1
; else
...
(define (fib-iter a b p q count)
  (cond ((= count 0) b)
        ((even? count)
         (fib-iter a b
                   (if (= (+ p q) 1) 1 (- (* p (+ (* 2 p) q)) 1)) ; compute p'
                   (* q (+ (* 2 p) q)) ; compute q'
                   (/ count 2)))
        (else (fib-iter (+ (* b q) (* a q) (* a p))
                        (+ (* b p) (* a q)) p q (- count 1)))))